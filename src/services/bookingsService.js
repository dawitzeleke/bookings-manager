...
export async function getRollingWorkWindows(bookingSettings) {
    try {
        const rollingWorkingWindows = await BookingsManager.getRollingWorkWindows(bookingSettings);
...
export async function resolveEffectiveHours(bookingSettings) {
    try {
        const effectiveHours = await BookingsManager.resolveEffectiveHours(bookingSettings);
...
export async function getCreatorBookingSettings(creatorId) {
    try {
        const bookingSettings = await BookingsManager.getCreatorBookingSettings(creatorId);
...
export async function validateBookingDuration(creatorId,
    bookingStart,
    bookingEnd,
    bookingSettings) {
    try {
        const isValid = await BookingsManager.validateBookingDuration(creatorId, bookingStart, bookingEnd, bookingSettings);
        return isValid;
    } catch (error) {
...
export async function isBookingWithinOfflineHours(
    ...
    try {
      const isWithinOfflineHours = await BookingsManager.isBookingWithinOfflineHours(
        creatorId,
        bookingStartTime,
        bookingEndTime,
        date,
...
export async function calculatePrice(creatorId,
    appointmentStart,
    appointmentEnd,
    date,
...
export async function getUserBookingJson(creatorId, bookingSettings) {
    try {
        const bookingJson = await BookingsManager.getUserBookingJson(creatorId, bookingSettings);
        return bookingJson;
    } catch (error) {
...
export async function updateBookingSettings(userId, settingsObject, options){
    try {
        const updatedSettings = await BookingsManager.updateBookingSettings(userId, settingsObject, options);
...
export async function updateBookingStatus(bookingId, newStatus) {
    try {
        const updatedStatus = await BookingsManager.updateBookingStatus(bookingId, newStatus);
...
export async function setBookingStatus(bookingId, newStatus) {
    try {
        const updatedBooking = await BookingsManager.setBookingStatus(bookingId, newStatus);
...
export async function getUpcomingBookings(userId = null, windowInMinutes = 60){
    try {
       const upcomingBookings = await BookingsManager.getUpcomingBookings(userId, windowInMinutes);
...
export async function getUpcomingBookingSessions(creatorId) {
    try {
        const upcomingSessions = await BookingsManager.getUpcomingBookingSessions(creatorId);
...
export async function registerReadyState(bookingId, userType) {
    try {
        const result = await BookingsManager.registerReadyState(bookingId, userType);
        return result;
    } catch (error) {
