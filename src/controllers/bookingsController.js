...
export async function handleGetIsCreatorBookingSuspended(req, res) {
  try {
    ...
    const isSuspended = await isCreatorBookingSuspended(creatorId, date, bookingSettings);
...
export async function handleValidateBookingDuration(req, res) {
  try {
    ...
    const isValid = await validateBookingDuration(creatorId, bookingStart, bookingEnd, bookingSettings);
    res.json({ isValid });
  } catch (error) {
    console.error("Error:", error);
    res.status(500).send("Internal Server Error");
...
export async function handleIsBookingWithinOfflineHours(req, res) {
  try {
    ...
    const isWithinOfflineHours = await isBookingWithinOfflineHours(creatorId, bookingStartTime, bookingEndTime, date, bookingSettings);
...
export async function handleCalculatePrice(req, res) {
  try {
    ...
    const price = await calculatePrice(creatorId,
    appointmentStart,
    appointmentEnd,
    date,
    defaultTokenPerMinute,
...
export async function handleDoesAppointmentCrossOver(res, req) {
  try {
    const { creatorId, appointmentStart, appointmentEnd, date, bookingSettings } = req.body;

    // Assuming you have a service function to check for appointment crossover
    const doesCrossOver = await doesAppointmentCrossOver(creatorId, appointmentStart, appointmentEnd, date, bookingSettings);
...
export async function handleIsRequestedTimeAvailable(req, res) {
  try {
    ...
    const isAvailable = await isRequestedTimeAvailable(creatorId, requestedStart, requestedEnd, recurrenceRule);
...
export async function handleGetUserBookingJson(req, res){
    ...
    try {
        const userBookingJson = await getUserBookingJson(creatorId, bookingSettings);
...
export async function handleUpdateBookingSettings(req, res){
    ...
    try {
        const updatedSettings = await updateBookingSettings(userId, settingsObject, options);
